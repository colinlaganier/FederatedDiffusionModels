...
def train(config):
    ...
    # Enable automatic mixed precision within Ray
    train.torch.accelerate(amp=True)
    nnet = train.torch.prepare_model(train_state.nnet)
    nnet_ema = train.torch.prepare_model(train_state.nnet_ema)
    optimizer = train.torch.prepare_optimizer(train_state.optimizer)
    train_dataset_loader = train.torch.prepare_dataloader(train_dataset_loader)
    ...
    def get_data_generator():
        while True:
            for data in tqdm(train_dataset_loader, desc='epoch'):
                yield data
    ...
    def train_step(_batch):
        ...
        train.torch.backward(loss.mean())
